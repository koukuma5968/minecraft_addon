...
import { system as system3, TicksPerSecond as TicksPerSecond3, world as world6 } from "@minecraft/server";
...
function showSystemForm(player) {
  ...
  const time_limit = world3.getDynamicProperty("time_limit");
  modalForm.slider(
    { translate: "msg.kurokumaft:systemblock.gamerule.time_limit.label" },
    1,
    60,
    {
      defaultValue: time_limit !== void 0 ? time_limit : 10,
      valueStep: 1
    }
  );
  modalForm.divider();
  const transfer_position = world3.getDynamicProperty("transfer_position");
  modalForm.label(
    { translate: "msg.kurokumaft:systemblock.gamerule.transfer_position.label" }
  );
  modalForm.textField({ text: "x:" }, { rawtext: [{ text: transfer_position !== void 0 ? transfer_position.x.toString() : "" }] });
  modalForm.textField({ text: "y:" }, { rawtext: [{ text: transfer_position !== void 0 ? transfer_position.y.toString() : "" }] });
  modalForm.textField({ text: "z:" }, { rawtext: [{ text: transfer_position !== void 0 ? transfer_position.z.toString() : "" }] });
  modalForm.divider();
  const area_position = world3.getDynamicProperty("area_position");
  modalForm.label(
    { translate: "msg.kurokumaft:systemblock.gamerule.area_position.label" }
  );
  modalForm.textField({ text: "x:" }, { rawtext: [{ text: area_position !== void 0 ? area_position.x.toString() : "" }] });
  modalForm.textField({ text: "y:" }, { rawtext: [{ text: area_position !== void 0 ? area_position.y.toString() : "" }] });
  modalForm.textField({ text: "z:" }, { rawtext: [{ text: area_position !== void 0 ? area_position.z.toString() : "" }] });
  modalForm.divider();
  const area_range = world3.getDynamicProperty("area_range");
  modalForm.slider(
    { translate: "msg.kurokumaft:systemblock.gamerule.area_range.label" },
    30,
    240,
    {
      defaultValue: area_range !== void 0 ? area_range : 64,
      valueStep: 1
    }
  );
  modalForm.divider();
  const outcome = world3.getDynamicProperty("outcome");
  modalForm.dropdown(
    ...
    {
      defaultValueIndex: outcome !== void 0 ? outcome : 0
    }
  );
  modalForm.divider();
  modalForm.show(player).then((formData) => {
    ...
    if (values != void 0) {
      ...
      if (!eFlg) {
        ...
        world3.setDynamicProperty("time_limit", timeLimit);
        world3.setDynamicProperty("transfer_position", {
          x: Number(transferPositionX),
          y: Number(transferPositionY),
          z: Number(transferPositionZ)
        });
        world3.setDynamicProperty("area_position", {
          x: Number(areaPositionX),
          y: Number(areaPositionY),
          z: Number(areaPositionZ)
        });
        const areaRange = values[12];
        world3.setDynamicProperty("area_range", areaRange);
        const outcome2 = values[14];
        world3.setDynamicProperty("outcome", outcome2);
      } else {
        showSystemForm(player);
      }
    }
...
// scripts/item/StunGunComponent.ts
import { EntityComponentTypes } from "@minecraft/server";
var StunGunComponent = class {
  onUse(event, arg) {
...
// scripts/block/LockableBarsOn.ts
import { BlockPermutation, EntityComponentTypes as EntityComponentTypes2, EquipmentSlot, world as world4, system as system2, TicksPerSecond as TicksPerSecond2 } from "@minecraft/server";
    ...
    if (param.type === "lower") {
      ...
      dimension.setBlockPermutation(
        ...
        BlockPermutation.resolve(
          "kurokumaft:lockable_iron_bars_upper",
          {
      ...
      if (main !== void 0) {
        if (main.typeId === "kurokumaft:prison_cell_key") {
          ...
          if (param.type === "upper") {
            ...
            system2.waitTicks(10 * TicksPerSecond2).then(() => {
              block.dimension.setBlockPermutation(block.location, BlockPermutation.resolve("kurokumaft:lockable_iron_bars_upper", {
                "minecraft:cardinal_direction": block.permutation.getState("minecraft:cardinal_direction"),
                "kurokumaft:lock": "locking"
              }));
          ...
          players.forEach((p) => {
            if (p.getDynamicProperty("ensure")) {
              p.setDynamicProperty("ensure", void 0);
            }
          });
        }
      }
...
// scripts/block/WrapComponent.ts
import { world as world5 } from "@minecraft/server";
var WrapComponent = class {
  onTick(event, arg) {
    const block = event.block;
    const dimension = event.dimension;
    ...
    entities.forEach((entity) => {
      if (entity.getDynamicProperty("ensure")) {
        ...
        const tp_point = world5.getDynamicProperty("tp_point");
        if (tp_point !== void 0) {
...
world6.afterEvents.projectileHitEntity.subscribe((event) => {
  ...
  if (projectile !== void 0 && projectile.typeId === "kurokumaft:stun_bullet") {
    if (entity !== void 0) {
      entity.addEffect(MinecraftEffectTypes.Slowness, 5 * TicksPerSecond3, {
        amplifier: 5,
        showParticles: true
      });
...
world6.afterEvents.playerInteractWithEntity.subscribe((event) => {
  event.player;
});
