// scripts/kk_mh_script.ts
import { system as system9, world } from "@minecraft/server";

// scripts/items/ItemCustomBase.ts
...
// scripts/items/ItemCustomBase.ts
var ItemCustomBase = class {
  useEffectItem(entity, params) {
    if (params === void 0) {
      return;
    }
...
// scripts/items/MHUseItemEffectComponents.ts
var MHUseItemEffect = class extends ItemCustomBase {
  onConsume(event, arg) {
    super.useEffectItem(event.source, arg.params);
  }
};

// scripts/items/weapons/MHWeaponsItemComponents.ts
var MHWeaponsItem = class extends ItemCustomBase {
  onHitEntity(event, arg) {
  }
};

// scripts/items/armor/MHArmorsItemComponents.ts
var MHArmorsItem = class extends ItemCustomBase {
  onBeforeDurabilityDamage(event, arg) {
  }
};

// scripts/items/other/MHMeatRoastComponent.ts
import { EntityComponentTypes as EntityComponentTypes2, EquipmentSlot, ItemStack, system, TicksPerSecond as TicksPerSecond2 } from "@minecraft/server";
var MHMeatRoast = class {
  onUse(event, arg) {
    const item = event.itemStack;
    if (item !== void 0) {
      const player = event.source;
      const riding = player.getComponent(EntityComponentTypes2.Riding);
      if (riding === void 0) {
        return;
      }
      const ridEntity = riding.entityRidingOn;
      if (ridEntity !== void 0 && ridEntity.typeId === "kurokumaft:meat_roaster") {
        ...
        if (ridEntity.getProperty("kurokumaft:mh_action") === "none") {
          ...
          const run = system.runInterval(() => {
            ...
            if (startTime + 13 * TicksPerSecond2 < system.currentTick) {
            ...
            } else if (mainhand === void 0 || mainhand.typeId !== item.typeId) {
              ridEntity.setProperty("kurokumaft:mh_action", "none");
              player.playSound("meat_roast.open");
              player.playSound("meat_roast.failure");
              if (container.emptySlotsCount > 0) {
              ...
              if (container.contains(item)) {
                const slotNum = container.find(item);
            ...
            } else if (startTime + 11 * TicksPerSecond2 < system.currentTick) {
              ridEntity.setProperty("kurokumaft:roast_state", 3);
            } else if (startTime + 9 * TicksPerSecond2 < system.currentTick) {
              ridEntity.setProperty("kurokumaft:roast_state", 2);
            } else if (startTime + 4 * TicksPerSecond2 < system.currentTick) {
          ...
          player.playSound("meat_roast.open");
          ridEntity.setProperty("kurokumaft:mh_action", "none");
          const state = ridEntity.getProperty("kurokumaft:roast_state");
...
// scripts/blocks/MHPlantsGrowth.ts
import { BlockPermutation, EntityComponentTypes as EntityComponentTypes4, EquipmentSlot as EquipmentSlot3 } from "@minecraft/server";

// scripts/common/MHCommonUtil.ts
...
async function subtractionContainerItem(container, slot, decNum) {
...
// scripts/blocks/MHPlantsGrowth.ts
var MHPlantsGrowth = class {
  onTick(event, arg) {
    const block = event.block;
    const states = block.permutation.getAllStates();
    const growth = states["kurokumaft:growth"];
    const groew_types = arg.params;
    if (growth < groew_types.grow_size) {
      switch (groew_types.type) {
        case "farmland":
          ...
          break;
        case "log":
          const block_face = states["minecraft:block_face"];
          switch (block_face) {
            case "up":
              const up = block.dimension.getBlock({ x: block.location.x, y: block.location.y - 1, z: block.location.z });
              ...
              break;
            case "down":
              const down = block.dimension.getBlock({ x: block.location.x, y: block.location.y + 1, z: block.location.z });
              if (down.permutation.getTags().indexOf("log") != -1) {
                block.setPermutation(BlockPermutation.resolve(block.typeId, { "kurokumaft:growth": growth + 1, "minecraft:block_face": block_face }));
              }
              break;
            case "north":
              const north = block.dimension.getBlock({ x: block.location.x, y: block.location.y, z: block.location.z + 1 });
              if (north.permutation.getTags().indexOf("log") != -1) {
                block.setPermutation(BlockPermutation.resolve(block.typeId, { "kurokumaft:growth": growth + 1, "minecraft:block_face": block_face }));
              }
              break;
            case "south":
              const south = block.dimension.getBlock({ x: block.location.x, y: block.location.y, z: block.location.z - 1 });
              if (south.permutation.getTags().indexOf("log") != -1) {
                block.setPermutation(BlockPermutation.resolve(block.typeId, { "kurokumaft:growth": growth + 1, "minecraft:block_face": block_face }));
              }
              break;
            case "west":
              const west = block.dimension.getBlock({ x: block.location.x + 1, y: block.location.y, z: block.location.z });
              if (west.permutation.getTags().indexOf("log") != -1) {
                block.setPermutation(BlockPermutation.resolve(block.typeId, { "kurokumaft:growth": growth + 1, "minecraft:block_face": block_face }));
              }
              break;
            case "east":
              const east = block.dimension.getBlock({ x: block.location.x - 1, y: block.location.y, z: block.location.z });
            ...
            for (let x = -4; x <= 4; x++) {
              for (let z = -4; z <= 4; z++) {
                ...
                if (side.typeId == "minecraft:water") {
                  ...
                  subtractionItem(player, itemStack, EquipmentSlot3.Mainhand, 1);
                }
              }
            }
            break;
...
// scripts/blocks/MHRecipeBookCraft.ts
import { EntityComponentTypes as EntityComponentTypes5, ItemStack as ItemStack4 } from "@minecraft/server";
import { ModalFormData } from "@minecraft/server-ui";
var MHRecipeBookCraft = class {
  onPlayerInteract(event, arg) {
    const recipe_type = arg.params;
    const player = event.player;
    loopModal(player, recipe_type);
  }
};
async function loopModal(player, recipe_type) {
  let selectIndex = 0;
  let num = 1;
  while (true) {
    ...
    const container = inventory.container;
    const itme1_find = container.find(new ItemStack4(itme1));
    const itme2_find = container.find(new ItemStack4(itme2));
    if (itme1_find !== void 0 && itme2_find !== void 0) {
      const stack1 = container.getItem(itme1_find);
      const stack2 = container.getItem(itme2_find);
      if (stack1 !== void 0 && stack2 !== void 0 && (stack1.amount < num || stack2.amount < num)) {
      ...
      const rate = getRecipeRate(recipe_type.type, craftRecipe.rate);
      for (let i = 0; num > i; i++) {
        let craftItem = new ItemStack4("kurokumaft:non_combustible_waste", 1);
        if (Math.random() <= rate / 100) {
          craftItem = new ItemStack4(craftRecipe.result.id, 1);
        }
        if (container.emptySlotsCount > 0) {
...
function openModal(player, recipe_type, selectIndex, num) {
    ...
    modalForm.show(player).then((formData) => {
      if (!formData.canceled) {
        if (formData.formValues !== void 0) {
          resolve(
            {
              select: formData.formValues[0],
              num: formData.formValues[2]
            }
          );
        } else {
          resolve(null);
...
function getRecipeText(recipe_type) {
  const recipeText = [];
...
function getRecipeRate(recipe_type, rate) {
  switch (recipe_type) {
    case "master":
      rate = rate + 5;
    case "advanced":
...
// scripts/custom/MHCustomComponentRegistry.ts
function MHCustomComponentRegistry(initEvent) {
...
// scripts/items/MHItemFunction.ts
import { EntityComponentTypes as EntityComponentTypes6 } from "@minecraft/server";

// scripts/common/types/MHItemTypes.ts
var sharpnessDamageMap = [
  {
    sharpness: 10,
    rate: 0.1
  },
...
// scripts/items/MHItemFunction.ts
async function setItemLore(event) {
  const item = event.itemStack;
  if (item !== void 0) {
    if (item.hasTag("kurokumaft:mh_weapons")) {
      const lore = item.getLore();
      if (lore.length === 0) {
        const weponsCustom = item.getComponent("kurokumaft:mh_wepons_custom");
        const parameters = weponsCustom.customComponentParameters;
        const itemTypes = parameters.params;
        ...
        const player = event.player;
        const inventory = player.getComponent(EntityComponentTypes6.Inventory);
        const container = inventory.container;
      ...
      const lore = item.getLore();
      if (lore.length === 0) {
        const armorCustom = item.getComponent("kurokumaft:mh_armors_custom");
        const parameters = armorCustom.customComponentParameters;
        const armorTypes = parameters.params;
        const armorPropatie = armorPropatiesMap.find((pre) => pre.name === armorTypes.name && pre.type === armorTypes.type);
        if (armorPropatie !== void 0) {
          const skills = armorPropatie.skills;
          const skilPoint = [];
          skills.forEach((value) => {
            const skil = { rawtext: [{ translate: "skill.kurokumaft:" + value.name + ".name", with: [value.point.toString()] }] };
            skilPoint.push(skil);
          });
          const protection = armorTypes.protection;
          item.setLore(
            [
              { rawtext: [
...
// scripts/player/MHPlayerInputOperation.ts
import { ButtonState, InputButton } from "@minecraft/server";
function setGuardOperation(event) {
  const player = event.player;
...
// scripts/monster/MHMonsterFunction.ts
import { EntityComponentTypes as EntityComponentTypes7, EquipmentSlot as EquipmentSlot4 } from "@minecraft/server";

// scripts/monster/action/herbivore/AptonosAction.ts
import { ItemStack as ItemStack7, TicksPerSecond as TicksPerSecond4, system as system3 } from "@minecraft/server";

// scripts/monster/action/MHMonsterActionBase.ts
import { GameMode, system as system2, TicksPerSecond as TicksPerSecond3 } from "@minecraft/server";

// scripts/monster/MHMonsterManager.ts
var MHMonsterManager = class {
  static monsters = /* @__PURE__ */ new Map();
  static addMonster(monster, action) {
    this.monsters.set(monster.id, action);
    action.startPeaceTimeActionControl();
  }
  static removeMonster(id) {
    this.monsters.delete(id);
  }
  static damageMonster(hitEntity, damagingEntity) {
    const monster = this.monsters.get(hitEntity.id);
    if (monster !== void 0) {
      monster.damageAction(damagingEntity);
    }
  }
  static peelingMHMonster(target, itemStack) {
    if (itemStack !== void 0 && "kurokumaft:peeling_knife" === itemStack.typeId) {
...
// scripts/monster/action/MHMonsterActionBase.ts
var MHMonsterActionBase = class {
  // モブ
  entity;
  // ヘイト量
  hateEntitys = /* @__PURE__ */ new Map();
  // 攻撃力
  attack = 0;
  // スピード
  speed = 0;
  ...
  startPeaceTimeActionControl() {
    if (this.entity.getProperty("kurokumaft:mh_action") === "dai") {
      system2.waitTicks(30 * TicksPerSecond3).then(() => {
    ...
    this.peaceTimeNumber = system2.runInterval(() => {
      try {
        if (this.entity.isValid) {
          if (this.entity.getProperty("kurokumaft:mh_action") === "dai") {
            system2.clearRun(this.peaceTimeNumber);
          } else {
  ...
  startHostileActionControl() {
    this.hostileNumber = system2.runInterval(() => {
      try {
        if (this.entity.isValid) {
          if (this.entity.getProperty("kurokumaft:mh_action") === "dai") {
            system2.clearRun(this.hostileNumber);
          } else {
            this.hostileAction();
          }
        } else {
  ...
  peaceTimeAction() {
    if (this.lastMoveTime === 0) {
      this.lastMoveTime = system2.currentTick;
      this.entity.setProperty("kurokumaft:mh_action", "walk");
    }
    if (this.checkDaiState()) {
      return;
    }
    const mh_action_status = this.entity.getProperty("kurokumaft:mh_action_status");
    if (mh_action_status === "peacetime") {
      if (this.lastMoveTime < system2.currentTick) {
  ...
  hostileAction() {
    ...
    if (mh_action_status === "hostile") {
      if (this.lastdamageTime < system2.currentTick) {
        this.lastdamageTime = 0;
        this.lastMoveTime = system2.currentTick + 0.5 * TicksPerSecond3;
        system2.clearRun(this.hostileNumber);
        this.hostileNumber = 0;
        this.entity.setProperty("kurokumaft:mh_action_status", "peacetime");
        this.startPeaceTimeActionControl();
      } else if (this.lastMoveTime < system2.currentTick) {
  ...
  peelingDrop() {
    if (this.entity.getProperty("kurokumaft:mh_action") === "dai") {
      try {
        if (this.entity.isValid) {
          const peeling_count = this.entity.getProperty("kurokumaft:peeling_count");
          if (peeling_count !== 0) {
  ...
  setAilmentDamage(damagingEntity, weakness) {
    const ailment_state = this.entity.getProperty("kurokumaft:ailment_state");
    if (!ailment_state) {
      const ailmentDamage = getMHMonsterAilmentDamage(damagingEntity, weakness);
      if (ailmentDamage === void 0) {
        return;
      }
      let ailment_stateflg = false;
      switch (ailmentDamage.type) {
        case "poison":
          const poison_gauge = this.entity.getProperty("kurokumaft:poison_gauge");
          ...
          break;
        case "paralysis":
          const paralysis_gauge = this.entity.getProperty("kurokumaft:paralysis_gauge");
          ...
          break;
        case "sleep":
          const sleep_gauge = this.entity.getProperty("kurokumaft:sleep_gauge");
      ...
      if (ailment_stateflg) {
        const lastAlimentTime = system2.currentTick + ailmentDamage.duration;
        this.ailmentTick = system2.runInterval(() => {
          const health = this.entity.getProperty("kurokumaft:health");
          if (health === 0 || system2.currentTick > lastAlimentTime) {
            ...
            system2.clearRun(this.ailmentTick);
            this.ailmentTick = 0;
            return;
          }
  ...
  setDizziness(damagingEntity, weakDizziness) {
    ...
    if (dizziness_gauge < 100) {
      ...
      if (dizziness_gauge + dizzinessDamage.endurance >= 100) {
        ...
        this.entity.setProperty("kurokumaft:mh_action", "dizziness");
        this.entity.setProperty("kurokumaft:mh_action_status", "inactive");
        const lastAlimentTime = system2.currentTick + dizzinessDamage.duration;
        this.dizzinessTick = system2.runInterval(() => {
          if (this.checkDaiState() || system2.currentTick > lastAlimentTime) {
            if (this.entity.isValid) {
              this.entity.setProperty("kurokumaft:dizziness_gauge", 0);
              this.entity.setProperty("kurokumaft:mh_action_status", "hostile");
            }
            system2.clearRun(this.dizzinessTick);
  ...
  moveTowards(targetLoc, speed, stopDistance) {
    const thisLoc = this.entity.location;
    const dx = targetLoc.x - thisLoc.x;
    const dz = targetLoc.z - thisLoc.z;
    const dist = Math.sqrt(dx * dx + dz * dz);
  ...
  searchPlayer(location, maxDistance) {
...
var MHAptonosAilmentTypes = {
  attack: ["none"],
  weakness: [
    { type: "poison", endurance: 2, duration: 30 * TicksPerSecond4, damage: 2 },
    { type: "paralysis", endurance: 2, duration: 5 * TicksPerSecond4, damage: 0 },
    { type: "sleep", endurance: 2, duration: 15 * TicksPerSecond4, damage: 0 },
    { type: "dizziness", endurance: 2, duration: 5 * TicksPerSecond4, damage: 0 }
  ]
};
var MHAptonosPeaceTimeAction = weightChoice([
  { weight: 0.4, choice: "walk" },
  { weight: 0.2, choice: "wait" },
  { weight: 0.4, choice: "eat" }
]);
var MHAptonosPeelingItems = weightChoice([
  { weight: 0.8, choice: "kurokumaft:mh_meat" },
  { weight: 0.2, choice: "kurokumaft:small_keel" }
]);
var AptonosAction = class extends MHMonsterActionBase {
  intimidationAction() {
    throw new Error("Method not implemented.");
  }
  constructor(entity) {
    super(entity);
    this.hostileTimeBorder = 30 * TicksPerSecond4;
    this.speed = 0.06;
    this.hostileSpeed = 0.1;
  }
  individualPeaceTimeAction() {
    const action = MHAptonosPeaceTimeAction.pick();
    if ("walk" === action) {
      this.lastMoveTime = system3.currentTick + 5 * TicksPerSecond4;
      this.entity.setRotation(randomAngle());
    } else if ("wait" === action) {
      this.lastMoveTime = system3.currentTick + 1 * TicksPerSecond4;
    } else if ("eat" === action) {
      this.lastMoveTime = system3.currentTick + 2 * TicksPerSecond4;
    }
    this.entity.setProperty("kurokumaft:mh_action", action);
  }
  individualDamageCalculation(damagingEntity) {
    return hitMHMonsterDamage(damagingEntity, MHAptonosElementTypes.weakness);
  }
  individualAilment(damagingEntity) {
  ...
  individualHostileAction() {
    ...
    if (hateEntity === void 0) {
      ...
      this.lastMoveTime = system3.currentTick + 0.1 * TicksPerSecond4;
      this.entity.setProperty("kurokumaft:mh_action", "escape");
    }
  }
  choicePeelingItem() {
    const choice = MHAptonosPeelingItems.pick();
    return new ItemStack7(choice, 1);
  }
};

// scripts/monster/action/bird_dragon/RamposAction.ts
import { EntityDamageCause, ItemStack as ItemStack8, TicksPerSecond as TicksPerSecond5, system as system4 } from "@minecraft/server";
var MHRamposElementTypes = {
  attack: ["none"],
  weakness: [
    { type: "fire", endurance: 0.5 },
    { type: "water", endurance: 0.5 },
...
var MHRamposAilmentTypes = {
  attack: ["none"],
  weakness: [
    { type: "poison", endurance: 2, duration: 30 * TicksPerSecond5, damage: 1 },
    { type: "paralysis", endurance: 2, duration: 5 * TicksPerSecond5, damage: 0 },
    { type: "sleep", endurance: 2, duration: 15 * TicksPerSecond5, damage: 0 },
    { type: "dizziness", endurance: 2, duration: 15 * TicksPerSecond5, damage: 0 }
  ]
};
var MHRamposPeaceTimeAction = weightChoice([
  { weight: 0.4, choice: "walk" },
  { weight: 0.6, choice: "wait" }
]);
var MHRamposPeelingItems = weightChoice([
...
var RamposAction = class extends MHMonsterActionBase {
  constructor(entity) {
    super(entity);
    this.attack = 3;
    this.speed = 0.16;
    this.hostileSpeed = 0.16;
    this.hostileTimeBorder = 300 * TicksPerSecond5;
  }
  individualPeaceTimeAction() {
    ...
    if ("walk" === action) {
      this.lastMoveTime = system4.currentTick + 3 * TicksPerSecond5;
      this.entity.setRotation(randomAngle());
    } else if ("wait" === action) {
      this.lastMoveTime = system4.currentTick + 1 * TicksPerSecond5;
    }
    this.entity.setProperty("kurokumaft:mh_action", action);
  }
  individualHostileAction() {
    ...
    if (hateEntity === void 0) {
      ...
      if (this.entity.getProperty("kurokumaft:mh_action") === "wait") {
        this.lastMoveTime = system4.currentTick + 2 * TicksPerSecond5;
        if (targetDistance(this.entity.location, hateEntity.location) < 3) {
          ...
          system4.waitTicks(0.5 * TicksPerSecond5).then(() => {
      ...
      } else if (this.entity.getProperty("kurokumaft:mh_action") === "attack" || this.entity.getProperty("kurokumaft:mh_action") === "jump_attack") {
        this.entity.setProperty("kurokumaft:mh_action", "intimidation");
        this.lastMoveTime = system4.currentTick + 1.5 * TicksPerSecond5;
      } else {
        if (this.moveTowards(hateEntity.location, 0.32, 2)) {
          this.entity.setProperty("kurokumaft:mh_action", "wait");
          this.lastMoveTime = system4.currentTick + 1 * TicksPerSecond5;
        } else {
          this.lastMoveTime = system4.currentTick + 0.1 * TicksPerSecond5;
          this.entity.setProperty("kurokumaft:mh_action", "walk");
        }
      }
    }
  ...
  intimidationAction() {
    ...
    if (this.lastdamageTime === 0) {
      ...
      this.lastMoveTime = system4.currentTick + 1 * TicksPerSecond5;
      if (this.peaceTimeNumber !== 0) {
        this.lastdamageTime = system4.currentTick + this.hostileTimeBorder;
        this.entity.setProperty("kurokumaft:mh_action_status", "hostile");
        system4.clearRun(this.peaceTimeNumber);
        this.peaceTimeNumber = 0;
  ...
  choicePeelingItem() {
    ...
    return new ItemStack8(choice, 1);
  }
};

// scripts/monster/action/bird_dragon/DossRamposAction.ts
import { EntityDamageCause as EntityDamageCause2, ItemStack as ItemStack9, TicksPerSecond as TicksPerSecond6, system as system5 } from "@minecraft/server";
var MHRamposElementTypes2 = {
  attack: ["none"],
  weakness: [
    { type: "fire", endurance: 0.5 },
    { type: "water", endurance: 0.5 },
...
var MHRamposAilmentTypes2 = {
  attack: ["none"],
  weakness: [
    { type: "poison", endurance: 2, duration: 30 * TicksPerSecond6, damage: 1 },
    { type: "paralysis", endurance: 2, duration: 5 * TicksPerSecond6, damage: 0 },
    { type: "sleep", endurance: 2, duration: 15 * TicksPerSecond6, damage: 0 },
    { type: "dizziness", endurance: 2, duration: 15 * TicksPerSecond6, damage: 0 }
  ]
};
var MHRamposPeaceTimeAction2 = weightChoice([
  { weight: 0.4, choice: "walk" },
  { weight: 0.6, choice: "wait" }
]);
var MHRamposPeelingItems2 = weightChoice([
  { weight: 0.29, choice: "kurokumaft:rampos_scale" },
  { weight: 0.23, choice: "kurokumaft:rampos_fang" },
  { weight: 0.4, choice: "kurokumaft:rampos_skin" },
  { weight: 0.18, choice: "kurokumaft:small_keel" }
]);
var DossRamposAction = class extends MHMonsterActionBase {
  constructor(entity) {
    super(entity);
    this.attack = 3;
    this.speed = 0.16;
    this.hostileSpeed = 0.16;
    this.hostileTimeBorder = 300 * TicksPerSecond6;
  }
  individualPeaceTimeAction() {
    const attackTgergets = this.searchPlayer(this.entity.location, 16);
    if (attackTgergets.length > 0) {
    ...
    const action = MHRamposPeaceTimeAction2.pick();
    if ("walk" === action) {
      this.lastMoveTime = system5.currentTick + 3 * TicksPerSecond6;
      this.entity.setRotation(randomAngle());
    } else if ("wait" === action) {
      this.lastMoveTime = system5.currentTick + 1 * TicksPerSecond6;
    }
    this.entity.setProperty("kurokumaft:mh_action", action);
  }
  individualHostileAction() {
    ...
    if (hateEntity === void 0) {
      ...
      if (this.entity.getProperty("kurokumaft:mh_action") === "wait") {
        this.lastMoveTime = system5.currentTick + 2 * TicksPerSecond6;
        if (targetDistance(this.entity.location, hateEntity.location) < 5) {
          ...
          system5.waitTicks(0.5 * TicksPerSecond6).then(() => {
            const jumpAttackTgergets = this.entity.dimension.getEntities({
              families: ["player"],
              location: this.entity.location,
              maxDistance: 2
        ...
        this.entity.setProperty("kurokumaft:mh_action", "intimidation");
        this.lastMoveTime = system5.currentTick + 1.5 * TicksPerSecond6;
      } else {
        if (this.moveTowards(hateEntity.location, 0.32, 4)) {
          this.entity.setProperty("kurokumaft:mh_action", "wait");
          this.lastMoveTime = system5.currentTick + 1 * TicksPerSecond6;
        } else {
          this.lastMoveTime = system5.currentTick + 0.1 * TicksPerSecond6;
          this.entity.setProperty("kurokumaft:mh_action", "walk");
        }
      }
    }
  ...
  intimidationAction() {
    ...
    if (this.lastdamageTime === 0) {
      ...
      this.lastMoveTime = system5.currentTick + 1 * TicksPerSecond6;
      if (this.peaceTimeNumber !== 0) {
        this.lastdamageTime = system5.currentTick + this.hostileTimeBorder;
        this.entity.setProperty("kurokumaft:mh_action_status", "hostile");
        system5.clearRun(this.peaceTimeNumber);
        this.peaceTimeNumber = 0;
        super.startHostileActionControl();
      }
    }
  ...
  individualDamageCalculation(damagingEntity) {
    return hitMHMonsterDamage(damagingEntity, MHRamposElementTypes2.weakness);
  }
  individualAilment(damagingEntity) {
    this.setAilmentDamage(damagingEntity, MHRamposAilmentTypes2.weakness);
    this.setDizziness(damagingEntity, MHRamposAilmentTypes2.weakness[3]);
  }
  individualDaiAction() {
    this.entity.applyKnockback({
      x: -this.entity.getViewDirection().x * 2.5,
  ...
  choicePeelingItem() {
    const choice = MHRamposPeelingItems2.pick();
    return new ItemStack9(choice, 1);
  }
};

// scripts/common/types/MHMonsterTypes.ts
var MHHerbivoreMonsterActionClass = {
  aptonos: AptonosAction
};
var MHBirdDragonMonsterActionClass = {
  rampos: RamposAction,
  doss_rampos: DossRamposAction
};

// scripts/monster/MHMonsterFunction.ts
function startMHMonsterAction(entity) {
  const typeFamily = entity.getComponent(EntityComponentTypes7.TypeFamily);
  if (typeFamily !== void 0 && typeFamily.hasTypeFamily("mh_monster")) {
    const monsId = entity.typeId.substring(11);
    if (typeFamily !== void 0 && typeFamily.hasTypeFamily("herbivore")) {
    ...
    } else if (typeFamily !== void 0 && typeFamily.hasTypeFamily("bird_dragon")) {
...
function hitMHMonsterDamage(damagingEntity, weakElements) {
  let damage = 0;
  const equippable = damagingEntity.getComponent(EntityComponentTypes7.Equippable);
  const mainhand = equippable.getEquipment(EquipmentSlot4.Mainhand);
  if (mainhand !== void 0 && mainhand.hasTag("kurokumaft:mh_weapons")) {
    const weponsCustom = mainhand.getComponent("kurokumaft:mh_wepons_custom");
    const parameters = weponsCustom.customComponentParameters;
    const itemTypes = parameters.params;
...
function getMHMonsterAilmentDamage(damagingEntity, weakAilments) {
  const equippable = damagingEntity.getComponent(EntityComponentTypes7.Equippable);
  const mainhand = equippable.getEquipment(EquipmentSlot4.Mainhand);
  if (mainhand !== void 0 && mainhand.hasTag("kurokumaft:mh_weapons")) {
    const weponsCustom = mainhand.getComponent("kurokumaft:mh_wepons_custom");
    ...
    const alimentType = weakAilments.find((aliment) => aliment.type === elementType[0]);
    if (alimentType !== void 0) {
...
function getMHMonsterDizziness(damagingEntity, weakDizziness) {
  const equippable = damagingEntity.getComponent(EntityComponentTypes7.Equippable);
  const mainhand = equippable.getEquipment(EquipmentSlot4.Mainhand);
  if (mainhand !== void 0 && mainhand.hasTag("kurokumaft:mh_weapons")) {
    const weponsCustom = mainhand.getComponent("kurokumaft:mh_wepons_custom");
...
function getWeaponDamage(itemTypes, weakElements) {
  const rate = weaponsDamageMap.find((pre) => pre.weaponType === itemTypes.type);
  if (rate !== void 0) {
    ...
    const elementType = itemTypes.element;
    let element = 0;
    const weakElement = weakElements.find((element2) => element2.type === elementType[0]);
    if (weakElement !== void 0) {
...
// scripts/other/MHMeatRoasterManager.ts
import { EntityComponentTypes as EntityComponentTypes8, ItemStack as ItemStack11 } from "@minecraft/server";
function hitMHMeatRoaster(hitEntity, damagingEntity) {
  if (hitEntity.typeId === "kurokumaft:meat_roaster") {
    const damageFamily = damagingEntity.getComponent(EntityComponentTypes8.TypeFamily);
    if (damageFamily.hasTypeFamily("player")) {
      hitEntity.dimension.spawnItem(new ItemStack11("kurokumaft:meat_roaster", 1), hitEntity.location);
      hitEntity.remove();
    }
  }
}

// scripts/insect/MHInsectFunction.ts
import { EntityComponentTypes as EntityComponentTypes9 } from "@minecraft/server";

// scripts/insect/MHInsectManager.ts
var MHInsectManager = class {
  static insect = /* @__PURE__ */ new Map();
  static addInsect(insect, action) {
    this.insect.set(insect.id, action);
    action.startPeaceTimeActionControl();
  }
  static removeInsect(id) {
    this.insect.delete(id);
  }
  static damageInsect(hitEntity) {
    const monster = this.insect.get(hitEntity.id);
    if (monster !== void 0) {
      monster.damageAction();
    }
  }
  static huntingMHInsect(target, itemStack) {
    if (itemStack !== void 0 && "kurokumaft:insect_net" === itemStack.typeId) {
...
// scripts/insect/action/ImmortalInsectAction.ts
import { ItemStack as ItemStack13, TicksPerSecond as TicksPerSecond8, system as system7 } from "@minecraft/server";

// scripts/insect/action/MHInsectActionBase.ts
import { system as system6, TicksPerSecond as TicksPerSecond7 } from "@minecraft/server";
var MHInsectActionBase = class {
  // モブ
  entity;
  // 平常時スレッド番号
  peaceTimeNumber = 0;
  ...
  startPeaceTimeActionControl() {
    if (this.entity.getProperty("kurokumaft:mh_action") === "dai") {
      system6.waitTicks(30 * TicksPerSecond7).then(() => {
        if (this.entity.isValid) {
          this.entity.remove();
        }
    ...
    this.entity.setProperty("kurokumaft:mh_action_status", "peacetime");
    this.peaceTimeNumber = system6.runInterval(() => {
      try {
        if (this.entity.isValid) {
          if (this.entity.getProperty("kurokumaft:mh_action") === "dai") {
            system6.clearRun(this.peaceTimeNumber);
          } else {
            this.peaceTimeAction();
          }
        } else {
  ...
  peaceTimeAction() {
    if (this.lastMoveTime === 0) {
      this.lastMoveTime = system6.currentTick;
      this.entity.setProperty("kurokumaft:mh_action", "wait");
    }
    if (this.checkDaiState()) {
      return;
    ...
    if (mh_action_status === "peacetime") {
      const action = this.entity.getProperty("kurokumaft:mh_action");
      if (this.lastMoveTime - 20 < system6.currentTick) {
        this.individualPeaceTimeAction();
      } else {
        if ("move" === action) {
          if (this.lastRotaionTime < system6.currentTick) {
            this.entity.setRotation(randomAngle());
            this.lastRotaionTime = system6.currentTick + 4 * TicksPerSecond7;
            ;
          }
          this.entity.applyImpulse({
  ...
  setDaiAction() {
    try {
      ...
      system6.waitTicks(1 * TicksPerSecond7).then(() => {
        if (this.entity.isValid) {
          this.entity.remove();
        }
        MHInsectManager.removeInsect(this.entity.id);
...
// scripts/insect/action/ImmortalInsectAction.ts
var MHImmortalInsectTimeAction = weightChoice([
  { weight: 0.6, choice: "move" },
  { weight: 0.4, choice: "wait" }
]);
var MHImmortalInsectHuntingItem = "kurokumaft:immortal_insect_item";
var ImmortalInsectAction = class extends MHInsectActionBase {
  constructor(entity) {
    super(entity);
  }
  individualPeaceTimeAction() {
    ...
    if (this.lastMoveTime < system7.currentTick) {
      this.changeActionflg = false;
      if ("move" === this.changeAction) {
        this.lastMoveTime = system7.currentTick + 10 * TicksPerSecond8;
      } else if ("wait" === action) {
        this.moveLotationY = this.entity.location.y;
        this.lastMoveTime = system7.currentTick + 8 * TicksPerSecond8;
      }
  ...
  huntingItem() {
    return new ItemStack13(MHImmortalInsectHuntingItem);
  }
};

// scripts/insect/action/BitterBugAction.ts
import { ItemStack as ItemStack14, TicksPerSecond as TicksPerSecond9, system as system8 } from "@minecraft/server";
var MHImmortalInsectTimeAction2 = weightChoice([
  { weight: 0.6, choice: "move" },
  { weight: 0.4, choice: "wait" }
]);
var MHImmortalInsectHuntingItem2 = "kurokumaft:bitter_bug_item";
var BitterBugAction = class extends MHInsectActionBase {
  constructor(entity) {
    super(entity);
  }
  individualPeaceTimeAction() {
    const action = this.entity.getProperty("kurokumaft:mh_action");
    if (!this.changeActionflg) {
      this.changeAction = MHImmortalInsectTimeAction2.pick();
      this.changeActionflg = true;
    }
    if (this.lastMoveTime < system8.currentTick) {
      this.changeActionflg = false;
      if ("move" === this.changeAction) {
        this.lastMoveTime = system8.currentTick + 10 * TicksPerSecond9;
      } else if ("wait" === action) {
        this.moveLotationY = this.entity.location.y;
        this.lastMoveTime = system8.currentTick + 8 * TicksPerSecond9;
      }
      this.entity.setProperty("kurokumaft:mh_action", this.changeAction);
    }
    if (this.changeActionflg && "move" === action && "wait" === this.changeAction) {
  ...
  huntingItem() {
    return new ItemStack14(MHImmortalInsectHuntingItem2);
  }
};

// scripts/common/types/MHInsectTypes.ts
var MHInsectTypesActionClass = {
  immortal_insect: ImmortalInsectAction,
  bitter_bug: BitterBugAction
};

// scripts/insect/MHInsectFunction.ts
function startMHInsectAction(entity) {
  const typeFamily = entity.getComponent(EntityComponentTypes9.TypeFamily);
  if (typeFamily !== void 0 && typeFamily.hasTypeFamily("mh_insect")) {
...
//# sourceMappingURL=../debug/kk_mh_script.js.map
